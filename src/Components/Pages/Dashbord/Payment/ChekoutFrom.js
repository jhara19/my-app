...
const ChekoutFrom = ({
    ...
    return ( <
        form className = "my-4 p-10 w-full border-primary bg-white h-[200px]  border-2 rounded-xl"
        onSubmit = {
            handleSubmit
        } >
        <
        ...
        /> <
        button className = {
            `btn btn-sm w-full mt-8 bg-red-600 text-white ${
          loading && "loading"
        }`
        }
        type = "submit"
        ...
        /button> <
        p className = "text-center mt-6 text-red-600 font-bold" > {
            cardError
        } < /p> <
        /form>
    );
